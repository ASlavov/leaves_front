...
The tooltip that reads *"If the user already has entitled days for this year, they will be replaced"* is therefore **only true when adding for a single person**.
...
The current UI shows a static tooltip warning on the *Entitled Days* label. A more helpful flow would be:
...
2. If the count is > 0, show an inline confirmation message: *"X employee(s) already have entitlement for this leave type this year. Their records will be replaced. Continue?"*
