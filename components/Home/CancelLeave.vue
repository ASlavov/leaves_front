<template>
  <a
    href="#"
    class="py-3 px-5 inline-flex items-center gap-x-2 text-md dark:text-gray-100"
    ...
    <Transition name="modal">
      ...
      >
        <div class="sm:max-w-lg sm:w-full m-3 sm:mx-auto">
          <div
            class="w-full flex flex-col bg-white border shadow-sm rounded-xl dark:bg-neutral-800 dark:text-gray-100"
          >
            <button
              type="button"
            ...
            <div class="mx-auto font-extralight text-gray-500 dark:text-gray-100">
              {{ $t('leaves.selectLeaveToCancel') }}
            </div>
                    ...
                    <div v-for="leave in filteredLeavesData" :key="leave.id">
                      ...
                      >
                        <input
                          ...
                          :value="leave"
                        />
                        <span class="text-xs text-gray-400 mx-auto"
                          >{{ leave.start_date }} - {{ leave.end_date }}<br />
                          <span class="text-sm text-black font-semibold dark:text-gray-100"
                            >{{ calculateDays(leave.start_date, leave.end_date) }}
                            {{ $t('leaves.days') }}</span
                          >
                        </span>
                        <span
                          class="text-sm text-black-600 dark:text-neutral-400 mx-auto font-semibold"
                          >{{ getLeaveTypeName(leave.leave_type_id) }}</span
                        >
                        <span
                          class="text-sm ms-auto text-green-700 font-semibold dark:text-gray-100"
                          >{{
...
const leavesData = computed(() =>
  Array.isArray(leavesStore.leavesData?.currentUser) ? leavesStore.leavesData.currentUser : [],
);
