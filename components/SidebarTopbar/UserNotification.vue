<template>
  <div ref="dropdownContainer" class="relative">
    ...
    <div
      ...
      class="fixed top-[60px] right-0 h-full w-80 bg-white border border-gray-200 shadow-lg dark:bg-neutral-800 dark:border-neutral-600 overflow-y-auto z-[100]"
    >
      <!-- Tabs for Unread and Read -->
      <div
        :class="`flex justify-around py-2 border-b dark:border-neutral-600 ${theme === 'light' ? 'text-black' : 'text-white'}`"
      >
        <button
          :class="{ 'font-semibold': activeTab === 'unread' }"
        ...
        <button
          :class="{ 'font-semibold': activeTab === 'read' }"
          class="uppercase text-xs"
          @click="setTab('read')"
        >
          {{ $t('common.read') }}
      ...
      <ul class="divide-y divide-gray-200 dark:divide-neutral-600">
        <li
          ...
          :key="notification.id"
          class="p-4 text-sm text-gray-700 dark:text-gray-300 opacity-90 hover:opacity-100 cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-all"
          @click="changeNotificationStatus(notification.id)"
        >
