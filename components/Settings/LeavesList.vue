...
<!--            <div class="w-[50px] h-[50px] bg-gray-300 rounded-full mr-4 flex items-center justify-center col-span-1 ">
            ...
            <div class="col-span-2 flex " v-html="getUserIcons(leaveType.users)">
            </div>
            <div class="col-span-2" v-html="getDepartmentsText(leaveType.departments)">
            </div>
  ...
  */
  ...
  for(let i = 0; i < loops; i++) {
    ...
    if(user?.profile?.profile_image) {
      output += `<img src="${user.profile.profile_image}" />`
    } else {
      output += `
