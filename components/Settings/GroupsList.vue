          ...
          <div v-for="group in filteredGroups" :key="group.id" :class="group.users.length === 0 ? 'hidden' : ''">
            <div v-if="group.users.length" class="grid grid-cols-2 gap-[10px]">
              ...
              <div class="justify-self-end flex gap-[25px] items-center">
                <a v-if="permissionsStore.can('group','modify')" @click="editGroup(group.id)" class="cursor-pointer text-[#EA021A] font-bold underline">{{ $t('settings.editGroup') }}</a>
                <svg v-if="permissionsStore.can('group','modify')" @click="deleteGroup(group.id)" class="cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="16" height="19" viewBox="0 0 16 19" fill="none">
...
const modalComponent = computed(() => {
  // Assuming DeleteGroup exists or is handled similarly
