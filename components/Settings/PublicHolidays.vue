  ...
  <div class="px-[30px] pb-[30px] pt-[10px]">
    ...
    <div class="flex items-center gap-[12px] mb-[20px] flex-wrap">
      <div class="flex items-center gap-[8px]">
        <button
          class="p-[6px] rounded-full hover:bg-gray-100 dark:hover:bg-neutral-700 text-gray-600 dark:text-neutral-400 focus:outline-none"
          @click="prevYear"
        >
          <svg
            class="h-4 w-4"
        ...
        <span class="text-[15px] font-bold text-black dark:text-white w-[50px] text-center">{{
          selectedYear
        }}</span>
        <button
          class="p-[6px] rounded-full hover:bg-gray-100 dark:hover:bg-neutral-700 text-gray-600 dark:text-neutral-400 focus:outline-none"
      ...
      <template v-if="canModify">
        <button :class="submitBtnClass" @click="openAdd">+ {{ $t('settings.addHoliday') }}</button>
        <button
          class="inline-flex items-center justify-center py-[15px] px-[20px] rounded-[70px] border border-[#DFEAF2] dark:border-neutral-600 text-[14px] font-bold text-gray-700 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-neutral-700 focus:outline-none"
          @click="openMassAdd"
        >
    ...
    <div v-if="loading" class="text-[14px] text-[#808080] dark:text-neutral-400">
      {{ $t('common.loading') }}
    </div>

    <div
      v-else-if="yearHolidays.length === 0"
    ...
    <div
      v-else
      class="rounded-[8px] border border-[#DFEAF2] dark:border-neutral-600 overflow-hidden"
    >
      <!-- Header -->
      <div
        class="grid items-center px-[14px] py-[8px] bg-gray-50 dark:bg-neutral-800 text-[12px] font-bold text-[#808080] dark:text-neutral-400 uppercase tracking-wide"
      ...
      >
        ...
        <span class="text-[#808080] dark:text-neutral-400 pr-[16px]">{{
          formatDate(holiday.date)
        }}</span>
        <div class="flex items-center gap-[12px]">
          <template v-if="canModify">
            <button
              class="text-blue-500 hover:text-blue-700 dark:text-blue-400 focus:outline-none text-[12px]"
              @click="openEdit(holiday)"
            >
              {{ $t('common.edit') }}
            </button>
            <button
              class="text-red-500 hover:text-red-700 dark:text-red-400 focus:outline-none text-[12px]"
    ...
    >
      <div
        class="bg-white dark:bg-neutral-800 rounded-[12px] p-[24px] w-full max-w-[400px] shadow-xl"
      >
        ...
        <div class="space-y-[14px]">
          <div>
            <label :class="labelClass"
              >{{ $t('settings.holidayName') }} <span class="text-[#EA021A]">*</span></label
            >
            <input
              v-model="form.name"
              type="text"
          ...
          <div class="flex items-center gap-[10px] pt-[4px]">
            <button
              ...
              :class="[
                ...
                form.isRecurring ? 'bg-[#EA021A]' : 'bg-gray-200 dark:bg-neutral-600',
              ]"
            ...
            <span
              class="text-[13px] text-black dark:text-gray-100 cursor-pointer select-none"
              @click="form.isRecurring = !form.isRecurring"
            >
          ...
          <p class="text-[12px] text-[#808080] dark:text-neutral-400 -mt-[6px]">
            {{
              form.isRecurring
                ? $t('settings.holidayRecurringDesc')
                : $t('settings.holidayOneTimeDesc')
            }}
          </p>
        </div>
        <div class="flex gap-[10px] mt-[24px]">
          <button
            :disabled="formLoading || !form.name || !form.date"
    ...
    <div
      v-if="showMassAdd"
      class="fixed inset-0 z-[80] flex items-center justify-center bg-black bg-opacity-50"
      @click.self="closeMassAdd"
    >
      <div
        class="bg-white dark:bg-neutral-800 rounded-[12px] p-[24px] w-full max-w-[480px] shadow-xl"
      >
        <h3 class="text-[16px] font-bold text-black dark:text-white mb-[6px]">
          {{ $t('settings.massAddHolidays') }}
        </h3>
        <p class="text-[13px] text-[#808080] dark:text-neutral-400 mb-[20px]">
          {{ $t('settings.massAddDesc') }}
        </p>
        <div class="space-y-[14px]">
          <!-- Multi-date picker -->
          <div>
            ...
            <input
              ...
              type="text"
              :class="inputClass"
              :placeholder="$t('settings.clickToSelectDates')"
              readonly
            />
            <p
              v-if="massSelectedDates.length > 0"
              class="mt-[6px] text-[12px] text-[#808080] dark:text-neutral-400"
            >
          ...
          <div>
            <label :class="labelClass"
              ...
              <span class="text-[12px] font-normal text-[#808080]"
                >({{ $t('settings.leaveAllEmpty') }})</span
              ></label
            >
            <input
          ...
          <div class="flex items-center gap-[10px]">
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                massForm.isRecurring ? 'bg-[#EA021A]' : 'bg-gray-200 dark:bg-neutral-600',
              ]"
            ...
            >
              <span
                :class="[
                  'pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow transform transition duration-200 ease-in-out',
                  massForm.isRecurring ? 'translate-x-4' : 'translate-x-0',
                ]"
              />
            </button>
            <span
        ...
        <div class="flex gap-[10px] mt-[24px]">
          <button
            :disabled="massLoading || massSelectedDates.length === 0"
            :class="submitBtnClass"
            @click="saveMassAdd"
          >
            <svg
    ...
    >
      <div
        class="bg-white dark:bg-neutral-800 rounded-[12px] p-[24px] w-full max-w-[380px] shadow-xl"
      >
        <h3 class="text-[16px] font-bold text-black dark:text-white mb-[8px]">
          {{ $t('settings.deleteHoliday') }}
        </h3>
        <p class="text-[13px] text-[#808080] dark:text-neutral-400 mb-[20px]">
          {{ $t('settings.deleteHolidayConfirm', { name: deletingHoliday.name }) }}
        </p>
        ...
        <div class="flex gap-[10px]">
          <button
            ...
            class="inline-flex items-center justify-center py-[10px] px-[20px] rounded-[70px] bg-red-600 text-white text-[14px] font-bold hover:bg-red-700 focus:outline-none"
            @click="doDelete"
          >
            <svg
              v-if="deleteLoading"
    ...
    <div
      ...
      class="fixed inset-0 z-[80] flex items-center justify-center bg-black bg-opacity-50"
      @click.self="showMassDeleteConfirm = false"
    >
      <div
        class="bg-white dark:bg-neutral-800 rounded-[12px] p-[24px] w-full max-w-[380px] shadow-xl"
      >
        <h3 class="text-[16px] font-bold text-black dark:text-white mb-[8px]">
          {{ $t('settings.massDeleteHolidays') }}
        </h3>
        <p class="text-[13px] text-[#808080] dark:text-neutral-400 mb-[20px]">
          {{ $t('settings.massDeleteHolidaysConfirm', { count: selectedIds.size }) }}
        </p>
        <p class="text-[12px] text-amber-600 dark:text-amber-400 mb-[20px]">
          {{ $t('common.irreversibleAction') }}
        </p>
        <div class="flex gap-[10px]">
          ...
          <button
            class="inline-flex items-center justify-center py-[10px] px-[20px] rounded-[70px] border border-[#DFEAF2] dark:border-neutral-600 text-[14px] font-bold text-gray-700 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-neutral-700 focus:outline-none"
            @click="showMassDeleteConfirm = false"
          >
            {{ $t('common.cancel') }}
          </button>
        </div>
...
import { useHolidays } from '@/composables/holidaysApiComposable';
...
// Sync remote data to store (to keep existing store-based logic working if any)
watch(
  remoteHolidays,
  (newHolidays) => {
    if (newHolidays) holidaysStore.holidays = newHolidays;
  },
  { immediate: true },
);

// Recurring holidays appear in every year view; moving ones only in their own year
const yearHolidays = computed<PublicHoliday[]>(() =>
  (holidaysStore.holidays as PublicHoliday[]).filter(
    (h: PublicHoliday) =>
...
const allVisibleSelected = computed(
  () =>
    yearHolidays.value.length > 0 &&
    yearHolidays.value.every((h) => selectedIds.value.has(String(h.id))),
);
const someSelected = computed(
  () =>
    yearHolidays.value.some((h) => selectedIds.value.has(String(h.id))) &&
...
const editingHoliday = ref<PublicHoliday | null>(null);
...
const massDatePicker = ref<HTMLInputElement | null>(null);
const massSelectedDates = ref<string[]>([]);
const massForm = ref({ name: '', isRecurring: false });
const massLoading = ref(false);
let massPickerInstance: FlatpickrInstance | null = null;
...
const closeMassAdd = () => {
  ...
  if (massPickerInstance) {
    massPickerInstance.destroy();
    massPickerInstance = null;
  }
  massSelectedDates.value = [];
};
...
// ── Single Delete ─────────────────────────────────────
const deletingHoliday = ref<PublicHoliday | null>(null);
const deleteLoading = ref(false);

const confirmDelete = (holiday: PublicHoliday) => {
