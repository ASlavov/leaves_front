<template>
  ...
  >
    <div
      class="bg-white dark:bg-neutral-800 dark:text-white p-6 rounded-lg w-full max-w-md relative shadow-xl border dark:border-neutral-700"
    >
      <button
      ...
      <h2 class="text-xl font-bold mb-4 text-red-600 dark:text-red-500">
      ...
      <div class="space-y-4 text-sm border-t border-b py-4 dark:border-neutral-700">
        ...
        <div class="flex justify-between">
          <span class="font-bold uppercase text-xs text-gray-400">{{
...
const deleteEntitlement = async () => {
  ...
  try {
    ...
    useNuxtApp().$toast.error(
      type === 'user' && message ? message : t('settings.deleteEntitlementError'),
    );
  } finally {
    loading.value = false;
  }
