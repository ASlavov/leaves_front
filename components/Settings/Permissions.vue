      ...
      <div class="divide-y divide-gray-200 dark:divide-neutral-700">
        <div v-for="(category, categoryKey) in permissionCategories" :key="categoryKey" class="contents">
          ...
          <div v-for="(action, actionKey) in category.actions" :key="actionKey" class="grid grid-cols-1 md:grid-cols-5 p-4 items-center hover:bg-gray-50 transition-colors dark:hover:bg-neutral-700">
            ...
            <div v-for="role in roles" :key="role.key" class="md:col-span-1 flex justify-center py-2 md:py-0">
              <div class="w-6 h-6 rounded-full flex items-center justify-center transition-colors shadow-inner"
                   :class="hasPermission(role.key, categoryKey, actionKey) ? 'bg-green-100 text-green-600 dark:bg-green-900/40 dark:text-green-400' : 'bg-red-50 text-red-300 dark:bg-red-900/20 dark:text-red-800'">
                <svg v-if="hasPermission(role.key, categoryKey, actionKey)" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const hasPermission = (roleKey, category, action) => {
  ...
  return matrix[roleKey]?.[category]?.[action] || false;
};
</script>
