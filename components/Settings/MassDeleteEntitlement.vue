  ...
  <div class="px-[30px] pb-[30px] pt-[10px]">
      ...
      <div class="flex flex-wrap gap-[15px]">
        <div class="w-[300px]">
          <CustomSelect
            ...
            select-id="mass-delete-leave-type"
          />
        </div>

        <div class="w-[300px]">
          <label :class="labelClass"
            >{{ $t('common.year') }} <span class="text-[#EA021A]">*</span></label
          >
          <input
            v-model.number="year"
            type="number"
        ...
        <div class="w-full">
          <label :class="labelClass">
            ...
            <span class="text-[12px] font-normal text-[#808080] ml-1"
        ...
        <div class="w-full pt-[15px]">
          <button
            :disabled="!leaveTypeId || !year || previewLoading"
    ...
    <template v-else-if="step === 2">
      <div class="flex flex-col gap-[16px]">
        <!-- Nothing found -->
        <div
          v-if="preview.safe_count === 0 && preview.blocked_count === 0"
        ...
        <template v-else>
          <!-- Safe count -->
          <div
            class="flex items-center gap-[10px] p-[14px] rounded-[8px] bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800"
          >
            <svg
          ...
          <div v-if="preview.blocked_count > 0" class="flex flex-col gap-[10px]">
            <div
              class="flex items-start gap-[10px] p-[14px] rounded-[8px] bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800"
            >
              <svg
                class="h-5 w-5 text-amber-500 shrink-0 mt-[1px]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
            ...
            >
              <div
                class="grid grid-cols-2 px-[14px] py-[8px] bg-gray-50 dark:bg-neutral-800 text-[12px] font-bold text-[#808080] dark:text-neutral-400 uppercase tracking-wide"
              >
          ...
          <div class="flex gap-[12px] pt-[8px]">
            ...
            >
              ...
              {{
                $t('settings.massDeleteConfirm', {
                  count: force ? preview.safe_count + preview.blocked_count : preview.safe_count,
                })
              }}
            </button>
            <button
    ...
    <template v-else-if="step === 3">
      <div class="flex flex-col items-center gap-[20px] py-[20px]">
        <div
          class="h-14 w-14 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center"
        >
          <svg
        ...
        <p class="text-[15px] font-bold text-black dark:text-white text-center">
          {{
            result.skipped > 0
              ? $t('settings.massDeleteSuccessSkipped', {
        ...
        <button :class="submitBtnClass" @click="emit('saved')">
          {{ $t('common.done') }}
        </button>
      </div>
    </template>
...
const userOptions = computed(() => userStore.allUsers.map((u) => ({ id: u.id, name: u.name })));
