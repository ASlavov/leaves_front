<template>
    <div class="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-neutral-900">
      <div
        class="forgot-password w-full max-w-md p-4 bg-white border shadow-sm rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70">
        <h2 class="text-2xl font-bold text-center text-gray-700 dark:text-gray-300 pb-10">{{ $t('auth.forgotPassword') }}</h2>
        <form class="space-y-6">
          <div>
            ...
            <input id="email" name="email" type="email" required
              ...
              :placeholder="$t('auth.emailPlaceholder')" aria-describedby="email-error">
        ...
        <div class="mt-4 text-center">
          <NuxtLink to="../auth/login" class="text-sm text-blue-700">{{ $t('auth.backToLogin') }}</NuxtLink>
        </div>
      </div>
    </div>
  </template>
