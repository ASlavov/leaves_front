<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-neutral-900">
    ...
    >
      <h2 class="text-2xl font-bold text-center text-gray-700 dark:text-gray-300 pb-10">
        {{ $t('auth.login') }}
      </h2>
      ...
      <form class="space-y-6" @submit="login">
        <div>
          <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >{{ $t('auth.email') }} <span class="text-[#EA021A]">*</span></label
          >
          <input
            id="email"
            v-model="email"
        ...
        <div>
          <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >{{ $t('auth.password') }} <span class="text-[#EA021A]">*</span></label
          >
        ...
        <div class="flex items-center justify-between">
          ...
          <NuxtLink to="../auth/forgot-password" class="text-sm text-blue-700">{{
            $t('auth.forgotPassword')
          }}</NuxtLink>
        </div>

        <div v-if="loading" class="bg-red-600 w-full rounded-full h-2 animate-loading-bar"></div>
