import { computed, resolveComponent, ref, shallowRef, unref, withCtx, createVNode, toDisplayString, mergeProps, useSSRContext } from 'vue';
import { ssrRenderComponent, ssrRenderAttrs, ssrRenderList, ssrRenderStyle, ssrInterpolate, ssrRenderClass } from 'vue/server-renderer';
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const leavesData = computed(() => {
      ...
      (_a = userLeaves.value) == null ? void 0 : _a.forEach((userLeaves2) => {
        if (Array.isArray(userLeaves2.leaves)) {
          userLeaves2 == null ? void 0 : userLeaves2.leaves.forEach((leave) => {
            var _a2;
            if (selectedName.value && !userLeaves2.name.includes(selectedName.value)) return;
            if (selectedDepartment.value && parseInt(userLeaves2.department_id) !== parseInt(selectedDepartment.value)) return;
            ...
            if (!leaveTypeMap.has(leave.leave_type_id)) {
              ...
              displayedLeaveTypes.value.push({
                ...
                name: (_a2 = leavesStore.leavesData.leavesTypes.filter((leaveType) => leaveType.id === leave.leave_type_id)[0]) == null ? void 0 : _a2.name
              });
            }
            returnArray.push({
              ...leave,
    ...
    computed(() => {
      var _a;
      let eventsArray = ((_a = leavesData.value) == null ? void 0 : _a.map((leave) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (unref(calendarApp)) {
        _push(`<div${ssrRenderAttrs(_attrs)}><div class="grid grid-cols-12 gap-4 mb-4 dark:text-white items-end"><div class="col-span-12 sm:col-span-3 dark:text-white">`);
        ...
        if (selectedDepartment.value || selectedLeaveType.value || selectedName.value) {
          _push(`<div class="col-span-12 sm:col-span-3 leading-[46px] justify-self-end"><button class="btn btn-secondary text-red-500"> \xD7 \u039A\u03B1\u03B8\u03B1\u03C1\u03B9\u03C3\u03BC\u03CC\u03C2 \u03C6\u03AF\u03BB\u03C4\u03C1\u03C9\u03BD </button></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div><div class="flex flex-wrap mb-4"><!--[-->`);
        ssrRenderList(displayedLeaveTypes.value, (type) => {
          _push(`<div class="flex items-center mr-4 mb-2 dark:text-white cursor-pointer opacity-90 hover:opacity-100"><span class="w-[15px] h-[15px] mr-2" style="${ssrRenderStyle("background-color:" + getTypeColor(type.id))}"></span><span class="text-[14px]">${ssrInterpolate(type.name)}</span></div>`);
        });
        _push(`<!--]--></div>`);
        _push(ssrRenderComponent(unref(ScheduleXCalendar), { "calendar-app": unref(calendarApp) }, {
          monthGridEvent: withCtx(({ calendarEvent }, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="${ssrRenderClass(getEventClass(calendarEvent))}" style="${ssrRenderStyle(getEventStyle(calendarEvent))}"${_scopeId}>${ssrInterpolate(calendarEvent.title)}</div>`);
            } else {
              return [
                createVNode("div", {
                ...
                }, toDisplayString(calendarEvent.title), 15, ["onClick"])
              ];
            }
          }),
          _: 1
        ...
        ssrRenderList(Array(5).fill(), (y) => {
          _push(`<div class="grid grid-cols-7 gap-4 divide-x divide-gray-200 dark:divide-gray-700"><!--[-->`);
          ssrRenderList(Array(7).fill(), (x) => {
            _push(`<div class="items-center justify-between pt-4 pl-4 mt-4"><div><div class="h-2.5 bg-gray-300 rounded-full dark:bg-gray-600 w-24 mb-2.5"></div><div class="w-32 h-2 bg-gray-200 rounded-full dark:bg-gray-700"></div></div><div class="h-2.5 bg-gray-300 rounded-full dark:bg-gray-700 w-12"></div></div>`);
          });
          _push(`<!--]--></div>`);
        });
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/Calendar/Calendar.vue");
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="w-full lg:ps-64 bg-gray-100 dark:bg-neutral-900 min-h-dvh-64 duration-300"><div class="p-4 sm:p-6 space-y-4 sm:space-y-6">`);
