...
import { ssrRenderAttrs, ssrGetDirectiveProps, ssrRenderList, ssrInterpolate, ssrRenderAttr } from 'vue/server-renderer';
    ...
    selectedOptions: {
      get() {
        return this.options.filter(
          (option) => this.modelValue.includes(option.id)
        );
      },
      set(newVal) {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if ($data.isOpen) {
    ...
    ssrRenderList($options.filteredOptions, (option) => {
      var _a;
      _push(`<div class="flex items-center py-2 px-4 w-full text-sm text-gray-800 cursor-pointer hover:bg-gray-100 rounded-lg focus:outline-none focus:bg-gray-100 dark:hover:bg-neutral-800 dark:text-neutral-200 dark:focus:bg-neutral-800"><div class="size-8 me-2">${(_a = option.icon) != null ? _a : ""}</div><div><div class="text-sm font-semibold text-gray-800 dark:text-neutral-200">${ssrInterpolate(option.name)}</div><div class="text-xs text-gray-500 dark:text-neutral-500">${ssrInterpolate(option.description)}</div></div><div class="ms-auto">`);
      if ($options.isSelected(option)) {
        _push(`<span><svg class="shrink-0 size-4 fill-blue-600 text-blue-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M12.736 3.97a.733.733 0 011.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 01-1.065.02L3.217 8.384a.757.757 0 010-1.06.733.733 0 011.047 0l3.052 3.093 5.4-6.425a.247.247 0 01.02-.022Z"></path></svg></span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div>`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/misc/CustomMultiSelect.vue");
