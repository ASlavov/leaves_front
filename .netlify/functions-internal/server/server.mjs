...
import {
  w as withQuery,
  u as useNitroApp,
  n as normalizeLambdaIncomingHeaders,
  a as normalizeCookieHeader,
  b as normalizeLambdaOutgoingBody,
  c as normalizeLambdaOutgoingHeaders,
  g as getRouteRulesForPath,
...
const handler = async function handler2(event, context) {
  ...
  if (routeRules.isr) {
    ...
    const builderHandler = ttl
      ? (event2, context2) => lambda(event2, context2).then((r) => ({ ...r, ttl }))
      : lambda;
