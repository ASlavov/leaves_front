...
export const useEntitlementStore = defineStore('entitlementStore', () => {
  const entitledDaysData = ref<{
    savedUsers: Record<string | number, Record<string | number, Entitlement[]>>;
  }>({
    savedUsers: {},
  });
  ...
  async function getEntitledDaysForUser(
    userId: string | number,
    forceRefresh = false,
  ): Promise<Record<number | string, Entitlement[]> | undefined> {
  ...
  async function deleteEntitledDaysForUser(
    userId: string | number,
    entitlementId: string | number,
  ) {
    try {
